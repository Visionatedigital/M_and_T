...
async function checkApp() {
    ...
    try {
        ...
        const { rows } = await db.query('SELECT id, status, full_name FROM loan_applications WHERE id = $1', [id]);
        if (rows.length > 0) {
            console.log('Found application:', rows[0]);
        } else {
            console.log('Application NOT FOUND in database.');

            // List all IDs to see if it's there but slightly different?
