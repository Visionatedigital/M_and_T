...
async function checkGroupIdType() {
    const { data, error } = await supabase.from('loan_applications').select('group_id').limit(1);
    if (error) {
        console.error(error);
    } else {
        console.log("Sample group_id:", data[0]?.group_id);
        console.log("Type:", typeof data[0]?.group_id);
    }
}
