...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (laCols.has('borrower_id') && borrower_id) {
            cols.push('borrower_id');
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (body.borrower_id) {
            extraSql += `, borrower_id = $${p}`;
            values.push(body.borrower_id);
            p += 1;
        }
        if (isAdmin(req.user?.role)) {
...
// Clears duplicate / double-entry loans. Removes related repayments and
