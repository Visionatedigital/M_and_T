...
const ActiveLoans = () => {
  ...
  const loadActiveLoans = async () => {
    try {
      ...
      if (isSupabaseOffline) {
        console.log("🛠️ Loading active loans from local API...");
        data = await api.applications.getActive();
      } else {
      ...
      const processedLoans = data.map((loan: any) => {
        ...
        const currentGrowth = principal > 0 ? ((amountPaid - (principal * (Math.min(monthsElapsed, duration) / duration))) / principal) * 100 : 0;
