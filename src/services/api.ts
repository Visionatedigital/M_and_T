// ── API base URL ───────────────────────────────────────────
// - Dev: local Express on :3000 (or VITE_REMOTE_API_URL / VITE_USE_SUPABASE overrides).
// - Prod browser: same origin as the site (nginx proxies /api to Node).
...
function resolveApiOrigin(): string {
    ...
    if (import.meta.env.DEV) {
        return useSupabase ? DEV_REMOTE_DEFAULT : LOCAL_API;
    }
...
console.log(`🌐 API origin: ${_apiOrigin}${import.meta.env.DEV ? ' (dev)' : ' (prod)'} → ${API_URL}`);
